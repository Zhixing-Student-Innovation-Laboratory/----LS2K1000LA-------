...
set rc [catch {
  open_project {WYSWD.prj}
  import_device eagle_s20.db -package EG4S20BG256
  elaborate -top {WYSWD}
  export_db {WYSWD_elaborate.db}
} RESULT]
if {$rc} {
...
set rc [catch {
  read_adc ../../PIN/pin.adc
  optimize_rtl
  report_area -file WYSWD_rtl.area
  export_db {WYSWD_rtl.db}
} RESULT]
if {$rc} {
  step_error opt_rtl
  return -code error $RESULT
...
set rc [catch {
  optimize_gate -maparea WYSWD_gate.area
  legalize_phy_inst
  export_db {WYSWD_gate.db}
} RESULT]
if {$rc} {
  step_error opt_gate
  return -code error $RESULT
